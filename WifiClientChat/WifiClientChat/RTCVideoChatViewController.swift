...
class RTCVideoChatViewController: UIViewController,RTCEAGLVideoViewDelegate,WebrtcManagerProtocol,ServiceManagerProtocol {
  ...
  @IBAction func audioButtonPressed (_ sender:UIButton){
    ...
    {
        self.webrtcManager.removeAudio()
    }
    else
    {
    ...
    //self.webrtcManager.peerConnection?.localStreams.remove(at: 1)
  ...
  func videoView(_ videoView: RTCEAGLVideoView, didChangeVideoSize size: CGSize) {
    ...
    UIView.animate(withDuration: 0.4, animations: { () -> Void in
      ...
      if videoView == self.localView {
        ...
        //self.remoteViewTopConstraint!.constant = 1
        //self.remoteViewBottomConstraint!.constant = 1
        //self.remoteViewLeftConstraint!.constant = 1
        //self.remoteViewRightConstraint!.constant = 1
      }
      self.view.layoutIfNeeded()
    })
    }
